...
const VERSION = "v4";
const VERSION_LINK = "v3/"; // relative link to the previous stable release
...
/* ==================== v4 DATA: ratings, reviews, add-ons, schedule, payouts ==================== */
const RATINGS = {
  rest_oak_ash: { rating: 4.9, count: 1284 },
  rest_sweet_basil: { rating: 4.7, count: 863 },
  rest_kobu: { rating: 4.6, count: 702 },
  meal_shawarma_1: { rating: 4.8, count: 421 }, meal_salmon_2: { rating: 4.9, count: 388 },
  meal_teriyaki_3: { rating: 4.6, count: 254 }, meal_falafel_4: { rating: 4.7, count: 301 },
  meal_steak_5: { rating: 4.8, count: 337 }, meal_padthai_6: { rating: 4.5, count: 210 },
  meal_caesar_7: { rating: 4.4, count: 187 }, meal_chili_8: { rating: 4.6, count: 142 },
  meal_brisket_9: { rating: 4.7, count: 96 }, meal_harissa_10: { rating: 4.8, count: 133 },
  meal_skewer_11: { rating: 4.6, count: 118 }, meal_kofte_12: { rating: 4.7, count: 88 },
  meal_halloumi_13: { rating: 4.5, count: 74 }, meal_zaatar_14: { rating: 4.6, count: 91 },
  meal_tunapoke_15: { rating: 4.7, count: 156 }, meal_kungpao_16: { rating: 4.6, count: 129 },
  meal_yakisoba_17: { rating: 4.4, count: 83 }, meal_teriyakitofu_18: { rating: 4.5, count: 61 },
};
const REVIEWS = [
  { user: "Sam T.", rest: "rest_oak_ash", rating: 5, text: "The shawarma bowl is unreal. Arrived hot, huge portion, zero grease." },
  { user: "Priya N.", rest: "rest_sweet_basil", rating: 5, text: "Salmon was perfectly cooked and beautifully packed. So fresh." },
  { user: "Jordan K.", rest: "rest_kobu", rating: 4, text: "Teriyaki is my weekly staple. Would love a spicier option!" },
  { user: "Maya L.", rest: "rest_oak_ash", rating: 5, text: "Brisket mac is comfort food perfected. Consistent every week." },
  { user: "Dev P.", rest: "rest_sweet_basil", rating: 5, text: "Falafel stayed crispy on delivery. Big fan of the tahini." },
];
/* last week's box — for one-tap repeat */
const LAST_BOX = ["meal_salmon_2", "meal_teriyaki_3", "meal_steak_5", "meal_falafel_4", "meal_padthai_6", "meal_caesar_7"];
/* rolling schedule: next 4 weeks, each with per-week skip/pause state */
const SCHEDULE = [
  { date: "2026-08-18", week: "Aug 18", meals: 6, total: 78, status: "upcoming", window: "5:00–7:00 PM" },
  { date: "2026-08-25", week: "Aug 25", meals: 6, total: 78, status: "active", window: "6:00–8:00 PM" },
  { date: "2026-09-01", week: "Sep 01", meals: 6, total: 78, status: "active", window: "5:00–7:00 PM" },
  { date: "2026-09-08", week: "Sep 08", meals: 6, total: 78, status: "active", window: "7:00–9:00 PM" },
];
/* add-on / sides catalog */
const ADDONS = [
  { id: "add_protein", name: "Extra protein (+$4)", price: 4 },
  { id: "add_greens", name: "Side greens (+$3)", price: 3 },
  { id: "add_smoothie", name: "Protein smoothie (+$5)", price: 5 },
  { id: "add_dessert", name: "Chocolate blondie (+$3)", price: 3 },
];
/* delivery prefs (Instacart-style) */
const DELIVERY_PREFS = {
  dropoff: "CONCIERGE", leavePhoto: false, handTo: false, greenBag: true, contactless: true,
};
/* partner payout/analytics */
const PAYOUTS = {
  rest_oak_ash: { week: "$6,140", meals: 472, customers: 68, next: "Thu, Aug 20", est: "$6,140", yoy: "+18%" },
  rest_sweet_basil: { week: "$4,020", meals: 309, customers: 47, next: "Thu, Aug 20", est: "$4,020", yoy: "+12%" },
  rest_kobu: { week: "$3,120", meals: 240, customers: 38, next: "Thu, Aug 20", est: "$3,120", yoy: "+9%" },
};
const PAYOUT_BARS = [62, 78, 45, 90, 70, 100, 82]; // weekly revenue index (7 weeks)
...
function ratingHtml(id) {
  ...
  return `<span class="rating">${stars} <b>${r.rating}</b> <span class="muted">(${r.count.toLocaleString()})</span></span>`;
}
function starLine(rating) {
  const full = Math.round(rating);
  return "★★★★★".slice(0, full) + "☆☆☆☆☆".slice(0, 5 - full);
}
function addonPicker(mealId) {
  return ADDONS.map((a) => {
    const on = (addonsSelected[mealId] || []).includes(a.id);
    return `<button class="addon-opt ${on ? "on" : ""}" onclick="toggleAddon('${mealId}','${a.id}')">${esc(a.name)}</button>`;
  }).join("");
}
let addonsSelected = {}; // mealId -> [addonId]
function toggleAddon(mealId, addonId) {
  addonsSelected[mealId] = addonsSelected[mealId] || [];
  const i = addonsSelected[mealId].indexOf(addonId);
  if (i >= 0) addonsSelected[mealId].splice(i, 1); else addonsSelected[mealId].push(addonId);
  flash("✓ Add-on updated.");
  navigate();
}
function addonCost() {
  let total = 0;
  Object.entries(addonsSelected).forEach(([mealId, list]) => {
    list.forEach((id) => { const a = ADDONS.find((x) => x.id === id); if (a) total += a.price; });
  });
  return total;
}
...
/* ---------- router ---------- */
const routes = { "": renderHome, build: renderBuild, dashboard: renderDashboard, schedule: renderSchedule, track: renderTrack, partners: renderPartners, kitchen: renderKitchen, payouts: renderPayouts };
  ...
  return `
    <div class="mobile">
      ...
      <section class="card block">
        ...
        <div class="mini-map">
          <svg viewBox="0 0 300 150" class="map-svg">
            <path d="M20 130 L70 90 L130 100 L200 60 L250 40 L285 25" fill="none" stroke="#94a3b8" stroke-width="2.5" stroke-dasharray="5 5" stroke-linecap="round"/>
            <circle cx="20" cy="130" r="6" fill="#f59e0b"/><text x="30" y="134" font-size="10" fill="#64748b">Kitchen</text>
            <circle cx="285" cy="25" r="6" fill="#0d9488"/><text x="262" y="20" font-size="10" fill="#64748b">You</text>
            <circle cx="${courierMapX(TRACK.progress)}" cy="${courierMapY(TRACK.progress)}" r="7" fill="#0d9488" stroke="#fff" stroke-width="2"/>
            <text x="${courierMapX(TRACK.progress) - 20}" y="${courierMapY(TRACK.progress) - 12}" font-size="10" fill="#0d9488">${esc(TRACK.courierName)}</text>
          </svg>
        </div>
      </section>
      ...
      <section class="card block">
        ...
        <div class="courier-row">
          ...
          <button class="btn ghost sm" onclick="flash('Calling courier…')">${ico("tap")} Call</button>
          <button class="btn ghost sm" onclick="flash('Text sent to courier.')">${ico("menu")} Text</button>
        </div>
        <div class="cutoff" style="margin-top:12px">${ico("clock")} Next update in ~2 min · live</div>
      </section>

      <!-- delivery preferences + drop-off photo -->
      <section class="card block">
        <div class="kicker">${ico("shield")} Delivery preferences</div>
        <div class="pref-row">
          <span>Leave at door</span>
          <button class="toggle ${DELIVERY_PREFS.leavePhoto ? "on" : ""}" onclick="togglePref('leavePhoto')"></button>
        </div>
        <div class="pref-row">
          <span>Hand it to me</span>
          <button class="toggle ${DELIVERY_PREFS.handTo ? "on" : ""}" onclick="togglePref('handTo')"></button>
        </div>
        <div class="pref-row">
          <span>Reusable green bag</span>
          <button class="toggle ${DELIVERY_PREFS.greenBag ? "on" : ""}" onclick="togglePref('greenBag')"></button>
        </div>
        ${TRACK.status === "delivered" ? `
        <div class="dropoff-photo">
          <div class="photo-badge">${ico("check")} Delivered · photo taken</div>
          <svg viewBox="0 0 300 160" class="photo-svg">
            <rect width="300" height="160" fill="#e2e8f0"/>
            <rect x="120" y="70" width="60" height="45" rx="4" fill="#94a3b8"/>
            <rect x="110" y="62" width="80" height="10" rx="3" fill="#64748b"/>
            <text x="150" y="140" text-anchor="middle" font-size="11" fill="#64748b">Box left at door · 120 Bay St</text>
          </svg>
        </div>` : ""}
      </section>
...
function togglePref(key) { DELIVERY_PREFS[key] = !DELIVERY_PREFS[key]; flash("✓ Preference updated."); navigate(); }
...
function renderDashboard() {
    ...
    <div class="card meal">
      <div class="meal-top">
        <div>
          ...
          ${it.mealId ? `<div class="meal-rest">${ratingHtml(it.mealId)}</div>` : ""}
          ...
          <div class="addon-line">${ico("plus")} Add-ons
            <div class="addon-row">${addonPicker(it.mealId || "")}</div>
          </div>
        </div>
  ...
  return `
    <div class="mobile">
      ...
      <div class="actions">
        <button class="btn ghost col" onclick="repeatLastBox()">${ico("arrows")}<span>Repeat last box</span></button>
      ...
      <section class="card block">
        <div class="kicker">${ico("sparkle")} What others say</div>
        <div class="review-strip">
          ${REVIEWS.slice(0, 3).map((rv) => `<div class="review">
            <div class="rv-top"><b>${esc(rv.user)}</b><span class="rv-stars">${starLine(rv.rating)}</span></div>
      ...
      <section class="card block">
        ...
        <div class="billrow"><span>Add-ons &amp; sides</span><span class="bold">${money(addonCost())}</span></div>
        <div class="billrow"><span>Delivery, service fees &amp; taxes</span><span class="accent bold">INCLUDED ($0.00)</span></div>
        <div class="autocharge">${ico("clock")} Auto-charging ${money(total + addonCost())} on ${fmtDate(order.cutoffAt)} at 11:59 PM.</div>
      </section>
    </div>
    <div class="mobnav">
...
/* ---- repeat last box (one-tap reorder) ---- */
function repeatLastBox() {
  const order = DATA.dashboard.order;
  order.items = LAST_BOX.map((mealId, i) => {
    const m = DATA.meals.find((x) => x.id === mealId) || DATA.meals[i % DATA.meals.length];
    return { slot: i + 1, mealId: m.id, restaurantId: m.restaurantId, title: m.title, calories: m.calories, proteinGrams: m.proteinGrams, badges: m.badges };
  });
  order.totalAmount = order.items.length * 13;
  flash("✓ Repeated last week's box.");
  navigate();
}

/* ==================== SUBSCRIBER · ROLLING SCHEDULE ==================== */
function toggleWeek(i, field) {
  SCHEDULE[i][field] = !SCHEDULE[i][field];
  flash("✓ Schedule updated.");
  navigate();
}
function renderSchedule() {
  const rows = SCHEDULE.map((w, i) => {
    const skipped = w.status === "skipped", paused = w.status === "paused";
    return `<div class="card sched-row">
      <div class="sched-date"><span class="sched-week">${w.week}</span><span class="muted sm">${w.meals} meals · ${w.total} · ${w.window}</span></div>
      <div class="sched-actions">
        <button class="btn ghost sm ${skipped ? "on-skip" : ""}" onclick="toggleWeek(${i},'status')">${ico("pause")} ${skipped ? "Un-skip" : "Skip"}</button>
        <button class="btn ghost sm" onclick="flash('Swapped a meal for this week.')">${ico("swap")} Swap</button>
      </div>
    </div>`;
  }).join("");
  return `
    <div class="mobile">
      ...
      <section class="card block">
        <div class="kicker">${ico("calendar")} Delivery schedule</div>
        <div class="h2">Your next 4 weeks</div>
        <p class="muted sm">Skip, pause or swap any week ahead of its Sunday 11:59 PM cutoff. No forms, one tap.</p>
      </section>
      <div class="sched-list">${rows}</div>
      <section class="card block">
        <div class="kicker">${ico("bolt")} Auto-selection</div>
        <p class="muted sm">Don't choose? We pick meals from your dietary badges before each cutoff.</p>
      </section>
    </div>`;
}

/* ==================== PARTNER · PAYOUT DASHBOARD ==================== */
let payKitchen = "rest_oak_ash";
function setPayKitchen(id) { payKitchen = id; navigate(); }
function renderPayouts() {
  const p = PAYOUTS[payKitchen];
  const bars = PAYOUT_BARS.map((v, i) => `<div class="bar-col"><div class="bar-v" style="height:${v}%"></div><span class="bar-l">W${i + 1}</span></div>`).join("");
  const rows = [
    ["Gross revenue (this week)", p.week, "up"],
    ["Meals sold", String(p.meals), "flat"],
    ["Committed customers", String(p.customers), "up"],
    ["Avg revenue / customer", money(Math.round((parseFloat(p.week.replace(/[$,]/g, "")) / p.customers))), "up"],
  ].map(([label, val, dir]) => `<div class="pay-row"><span>${label}</span><span class="pay-val">${val} <span class="pay-dir ${dir}">${dir === "up" ? "▲" : "◆"}</span></span></div>`).join("");

  return `
    <div class="partner-shell">
      <header class="p-topbar">
        <div class="p-brand">${ico("store")}<div><b>Minimal Bites</b><span>partner payouts</span></div></div>
        <nav class="p-nav">
      ...
      <section class="p-filters">
        <label class="btn p-outline sm">${ico("store")} Kitchen
          <select onchange="setPayKitchen(this.value)">
            ${Object.keys(PAYOUTS).map((id) => `<option value="${id}" ${payKitchen === id ? "selected" : ""}>${esc(restName(id))}</option>`).join("")}
          </select></label>
        <button class="btn p-outline sm">${ico("calendar")} This week ▼</button>
      </section>

      <section class="pay-hero">
        <div class="pay-hero-label">Net payout · this week</div>
        <div class="pay-hero-amt">${p.week}</div>
        <div class="pay-hero-sub">Deposited ${p.next} · ${p.yoy} vs last month</div>
      </section>

      <section class="pay-grid">
        <div class="p-table-card pay-metrics"><div class="p-table-head"><span class="bold">${ico("chart")} Revenue & volume</span></div>${rows}</div>
        <div class="p-table-card pay-chart"><div class="p-table-head"><span class="bold">${ico("chart")} 7-week trend</span></div>
          <div class="chart-bars">${bars}</div></div>
      </section>

      <section class="p-controls card">
        <div class="p-label">Payouts</div>
        <div class="controls">
          <button class="btn p-primary" onclick="flash('Bank details updated.')">${ico("wallet")} Bank details</button>
          <button class="btn p-outline" onclick="flash('Payout report downloaded.')">${ico("download")} Export report</button>
        </div>
        <p class="muted sm" style="margin-top:12px">Automatic weekly payout every Thursday for every meal you confirmed &amp; cooked.</p>
      </section>
      <footer class="p-foot">Partner payouts — automatic weekly direct deposit.</footer>
    </div>`;
}
...
window.repeatLastBox = repeatLastBox;
window.toggleWeek = toggleWeek;
window.toggleAddon = toggleAddon;
window.addonsSelected = addonsSelected;
window.setPayKitchen = setPayKitchen;
